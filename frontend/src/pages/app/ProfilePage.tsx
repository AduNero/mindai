...
export default function ProfilePage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      const { data } = await usersApi.updateMyProfile({ bio: profile.bio });
      setProfile(data);
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="card flex items-center gap-4">
        <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full bg-brand-100 text-xl font-semibold text-brand-700 dark:bg-brand-950 dark:text-brand-300">
          {profile.profile_picture ? (
            ...
            profile.user.pseudonym[0]
          )}
        </div>
        <div>
          <p className="font-medium text-gray-900 dark:text-gray-100">{profile.user.pseudonym}</p>
          <p className="text-sm text-gray-500 dark:text-gray-400">Recovery email is private — not shown here.</p>
      ...
      <form onSubmit={handleSubmit} className="card space-y-4">
        <div>
          ...
          <textarea className="input" rows={3} value={profile.bio} onChange={handleBioChange} />
        </div>
