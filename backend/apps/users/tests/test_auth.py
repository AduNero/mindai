...
class TestEmailVerification:
    def test_verify_email_with_valid_token(self, api_client, user):
        user.is_email_verified = False
        user.save()
        token = EmailVerificationToken.objects.create(
            user=user, token="valid-token-123", expires_at=EmailVerificationToken.default_expiry()
        )

        response = api_client.post("/api/v1/auth/verify-email/", {"token": token.token})

        assert response.status_code == status.HTTP_200_OK
        user.refresh_from_db()
        assert user.is_email_verified is True

    def test_verify_email_with_invalid_token_fails(self, api_client):
        response = api_client.post("/api/v1/auth/verify-email/", {"token": "does-not-exist"})
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_verify_email_with_expired_token_fails(self, api_client, user):
        from django.utils import timezone

        token = EmailVerificationToken.objects.create(
            user=user, token="expired-token", expires_at=timezone.now() - timezone.timedelta(hours=1)
        )
        response = api_client.post("/api/v1/auth/verify-email/", {"token": token.token})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestPasswordReset:
    ...
    def test_confirm_reset_with_valid_token_changes_password(self, api_client, user):
        token = PasswordResetToken.objects.create(
            user=user, token="reset-token-123", expires_at=PasswordResetToken.default_expiry()
        )

        response = api_client.post(
            "/api/v1/auth/password-reset/confirm/",
            {"token": token.token, "new_password": "BrandNewPass99!", "new_password_confirm": "BrandNewPass99!"},
        )
    ...
    def test_confirm_reset_revokes_existing_sessions(self, api_client, user):
        ...
        token = PasswordResetToken.objects.create(
            user=user, token="reset-token-456", expires_at=PasswordResetToken.default_expiry()
        )
        api_client.post(
            "/api/v1/auth/password-reset/confirm/",
            {"token": token.token, "new_password": "BrandNewPass99!", "new_password_confirm": "BrandNewPass99!"},
